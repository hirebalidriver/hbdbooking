  ...
  <div>
    <div class="bg-white border border-gray-200 rounded-lg shadow-lg">
      <div class="card-body">
        ...
        <p>{{ adult }} adult x {{ $formatIDR(priceAdult?.idr_price || $convertToIDR(priceAdult?.price)) }}</p>
        <p v-if="child != 0">{{ child }} child x {{ $formatIDR(priceChild?.idr_price || $convertToIDR(priceChild?.price)) }}</p>
        <!-- <p>TOTAL USD 178</p> -->
        ...
        <div class="flex justify-between py-4 mt-4 border-t-2">
          ...
          <h2 class="text-lg font-bold" v-if="child != 0">
            {{ $formatIDR((adult * (priceAdult?.idr_price || $convertToIDR(priceAdult?.price))) + (child * (priceChild?.idr_price || $convertToIDR(priceChild?.price)))) }}
          </h2>
          <h2 class="text-lg font-bold" v-else>
            {{ $formatIDR(adult * (priceAdult?.idr_price || $convertToIDR(priceAdult?.price))) }}
          </h2>
        </div>
      </div>
    </div>
